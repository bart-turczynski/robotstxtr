...
// De-Abseil helpers.
//
// These replace the Abseil ASCII/string utilities the upstream matcher used
// (see PRD §6.1). They are behavior-neutral, ASCII-only, and byte-accurate:
// every predicate treats its argument as a single byte, so bytes outside the
// ASCII range classify exactly as Abseil's ascii_* helpers did (all false /
// unchanged), preserving the upstream matcher's byte-level behavior.
// ----------------------------------------------------------------------------
namespace {

inline bool AsciiIsLower(char c) { return c >= 'a' && c <= 'z'; }
inline bool AsciiIsUpper(char c) { return c >= 'A' && c <= 'Z'; }
inline bool AsciiIsAlpha(char c) { return AsciiIsLower(c) || AsciiIsUpper(c); }
inline bool AsciiIsDigit(char c) { return c >= '0' && c <= '9'; }
inline bool AsciiIsXDigit(char c) {
  return AsciiIsDigit(c) || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F');
}
...
inline char AsciiToUpper(char c) {
  return AsciiIsLower(c) ? static_cast<char>(c - ('a' - 'A')) : c;
}
inline char AsciiToLower(char c) {
  return AsciiIsUpper(c) ? static_cast<char>(c + ('a' - 'A')) : c;
}

// ASCII case-insensitive equality (replaces absl::EqualsIgnoreCase).
...
// Exact prefix match (replaces absl::StartsWith).
inline bool StartsWith(std::string_view text, std::string_view prefix) {
  return text.size() >= prefix.size() &&
         text.substr(0, prefix.size()) == prefix;
}

// ASCII case-insensitive prefix match (replaces absl::StartsWithIgnoreCase).
inline bool StartsWithIgnoreCase(std::string_view text,
                                 std::string_view prefix) {
  return text.size() >= prefix.size() &&
         EqualsIgnoreCaseAscii(text.substr(0, prefix.size()), prefix);
}

// Trim leading/trailing ASCII whitespace (replaces absl::StripAsciiWhitespace).
// Returns a view into the same buffer, matching the Abseil semantics the caller
// relies on.
inline std::string_view StripAsciiWhitespace(std::string_view s) {
  size_t start = 0;
  size_t end = s.size();
  while (start < end && AsciiIsSpace(s[start])) ++start;
...
static bool KeyIsUserAgent(std::string_view key, bool* is_acceptable_typo) {
  if (kAllowFrequentTypos) {
    *is_acceptable_typo = (StartsWithIgnoreCase(key, "useragent") ||
...
static bool KeyIsDisallow(std::string_view key, bool* is_acceptable_typo) {
  if (kAllowFrequentTypos) {
    *is_acceptable_typo = (StartsWithIgnoreCase(key, "dissallow") ||
...
static bool KeyIsSitemap(std::string_view key, bool* is_acceptable_typo) {
  if (kAllowFrequentTypos) {
    *is_acceptable_typo = StartsWithIgnoreCase(key, "site-map");
  } else {
    *is_acceptable_typo = false;
  }
...
void RobotsTxtParser::ParseAndEmitLine(int current_line, char* line,
  ...
  std::string_view key(string_key);
...
KeyType GetKeyType(std::string_view key, bool* is_acceptable_typo) {
...
void RobotsMatcher::HandleUserAgent(int line_num,
  ...
  if (user_agent.length() >= 1 && user_agent[0] == '*' &&
    ...
    for (const auto& agent : *user_agents_) {
      if (EqualsIgnoreCaseAscii(user_agent, agent)) {
